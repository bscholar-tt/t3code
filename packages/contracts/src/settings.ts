...
export const ClientSettingsSchema = Schema.Struct({
  ...
  desktopNotifyOnTurnCompleted: Schema.Boolean.pipe(
    Schema.withDecodingDefault(Effect.succeed(false)),
  ),
  desktopNotifyOnInputNeeded: Schema.Boolean.pipe(
    Schema.withDecodingDefault(Effect.succeed(false)),
  ),
  desktopNotifyOnError: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
});
...
export const PiSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("pi").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Pi Agent binary used by this instance.",
    ...
    homePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Pi HOME path",
        description: "Custom home directory for Pi Agent config and auth.",
        providerSettingsForm: { placeholder: "~/.pi", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
